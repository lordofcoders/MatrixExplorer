...
/**
 * A JTable with custom cell renderers for showing different visual representations of
 * the table data.
...
public class MatrixTable extends JTable {
    ...
    public JTableHeader getTableHeader() {
        ...
        header.setDefaultRenderer(new MatrixTableHeaderRenderer());
        return header;
    }
    ...
    public TableCellRenderer getCellRenderer(int row, int column){
        if (column == 0) {
            return new MatrixTableHeaderRenderer();
        } else {
            switch (renderMode) {
